...
def print_result(baysOpt,myfunction,Score,mybatch_type,acq_type,theta,toolbox='GPyOpt'):
...
def print_result_ystars(baysOpt,myfunction,Score,mybatch_type,acq_type,acq_params,theta,toolbox='GPyOpt'):
...
#    print('[{:s} {:s} {:s}] ElapseTime={:.3f}({:.2f})'.format(toolbox,mybatch_type,acq_type,np.mean(MyTime),np.std(MyTime)))
#    
#    if toolbox=='GPyOpt':
#        if myfunction.ismax==1:
#            print('MaxBest={:.4f}({:.2f})'.format(-1*np.mean(MaxFx),np.std(MaxFx)))    
...
#            print('OptTime/Iter={:.1f}({:.1f})'.format(np.mean(MyOptTime),np.std(MyOptTime)))
    ...
    if 'BatchSz' in Score:
        if acq_type['name']=='lei': # if ELI acquisition function, we have extra variable 'k'
            strFile="{:s}_{:d}_{:s}_{:s}_c_{:f}_B_{:d}_{}.pickle".format(myfunction.name,myfunction.input_dim,
                                                mybatch_type,acq_type['name'],acq_type['k'],int(BatchSz[0][1]),theta)
        else:
            strFile="{:s}_{:d}_{:s}_{:s}_B_{:d}_{}.pickle".format(myfunction.name,myfunction.input_dim,mybatch_type,acq_type['name'],int(BatchSz[0][1]))
        path=os.path.join(out_dir,strFile,theta)

        with open(path, 'wb') as f:
            pickle.dump([ybest, MyTime,BatchSz,baysOpt.bounds,MyOptTime], f)
    else:
        if acq_type['name']=='lei': # if ELI acquisition function, we have extra variable 'k'
            strFile="{:s}_{:d}_{:s}_{:s}_c_{:f}_{}.pickle".format(myfunction.name,myfunction.input_dim,
                                                mybatch_type,acq_type['name'],acq_type['k'],theta)
        else:
            strFile="{:s}_{:d}_{:s}_{:s}_{}.pickle".format(myfunction.name,myfunction.input_dim,mybatch_type,acq_type['name'],theta)
